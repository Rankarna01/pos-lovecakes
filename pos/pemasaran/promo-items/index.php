...
<body class="bg-slate-50 flex h-screen overflow-hidden text-slate-800 antialiased font-sans" x-data="promoItemsApp()" x-cloak>
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <header class="bg-primary text-white shadow-md px-4 sm:px-6 py-4 flex justify-between items-center z-20 shrink-0">
            <div class="flex items-center gap-4">
                ...
                <h2 class="text-xl font-black tracking-wide"><i class="fa-solid fa-gift mr-2"></i>Promo Beli X Gratis Y</h2>
            </div>
            ...
            <div class="w-full max-w-full space-y-6">
                ...
                <div class="bg-white p-3 rounded-2xl shadow-sm border border-slate-200 flex flex-col sm:flex-row gap-4 justify-between items-center sticky top-0 z-10">
                    <div class="relative w-full sm:w-80">
                        ...
                        <input type="text" x-model="searchQuery" placeholder="Cari nama promo atau produk..." class="w-full pl-11 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-primary/20 font-bold text-sm text-slate-700">
                    </div>
                        ...
                        <table class="w-full text-left border-collapse whitespace-nowrap">
                            <thead>
                                <tr class="bg-slate-50 border-b border-slate-200 text-[11px] text-slate-500 uppercase tracking-widest">
                                    <th class="p-4 font-black">Nama Promo</th>
                                    <th class="p-4 font-black">Syarat Pembelian (Beli X)</th>
                                    <th class="p-4 font-black">Hadiah Gratis (Gratis Y)</th>
                            ...
                            <tbody class="text-sm divide-y divide-slate-100">
                                <tr x-show="filteredData.length === 0">
                                    <td colspan="6" class="p-10 text-center">
                                        <div class="text-slate-300 text-5xl mb-3"><i class="fa-solid fa-gift"></i></div>
                                        <p class="text-slate-500 font-bold">Belum ada promo Beli X Gratis Y.</p>
                                    </td>
                                </tr>
                                ...
                                <template x-for="item in filteredData" :key="item.id">
                                    <tr class="hover:bg-slate-50/80 transition-colors">
                                        ...
                                        <td class="p-4">
                                            <span class="bg-blue-50 text-blue-700 font-bold px-3 py-1.5 rounded-xl border border-blue-200 text-xs inline-block">
                                                Beli <span class="font-black" x-text="item.buy_qty"></span> x <span x-text="item.buy_product_name"></span>
                                            </span>
                                        </td>
                                        <td class="p-4">
                                            <span class="bg-emerald-50 text-emerald-700 font-bold px-3 py-1.5 rounded-xl border border-emerald-200 text-xs inline-block">
                                                Gratis <span class="font-black" x-text="item.get_qty"></span> x <span x-text="item.get_product_name"></span>
                                            </span>
                                        </td>
        ...
        <div x-show="showModal" class="fixed inset-0 z-50 flex items-center justify-center" style="display: none;">
            ...
            <div class="bg-white w-full max-w-xl rounded-3xl shadow-2xl relative z-10 flex flex-col max-h-[90vh] overflow-hidden m-4 transform transition-all">
                <div class="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    <h3 class="font-black text-lg text-slate-800" x-text="isEdit ? 'Edit Promo Beli X Gratis Y' : 'Tambah Promo Baru'"></h3>
                ...
                <div class="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-5">
                    <div>
                        ...
                        <input type="text" x-model="form.name" placeholder="Misal: Beli 1 Roti Coklat Gratis 1 Roti Vanila" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 outline-none focus:bg-white focus:border-primary font-bold text-sm transition-colors" required>
                    </div>

                    <div class="p-4 bg-blue-50/50 rounded-2xl border border-blue-100 space-y-3">
                        <div class="text-xs font-black text-blue-700 uppercase flex items-center gap-1.5">
                            <i class="fa-solid fa-cart-shopping"></i> Syarat Pembelian (Beli X)
                        </div>
                        <div class="grid grid-cols-3 gap-3">
                            <div class="col-span-2">
                                <label class="block text-[10px] font-bold text-slate-500 mb-1 uppercase">Pilih Produk</label>
                                <select x-model="form.buy_product_id" class="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 outline-none focus:border-primary font-bold text-xs cursor-pointer">
                                    <option value="">-- Pilih Produk Syarat --</option>
                            ...
                            <div>
                                <label class="block text-[10px] font-bold text-slate-500 mb-1 uppercase">Jumlah (Qty)</label>
                                <input type="number" min="1" x-model="form.buy_qty" class="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 outline-none focus:border-primary font-bold text-xs text-center">
                            </div>
                        </div>
                    </div>

                    <div class="p-4 bg-emerald-50/50 rounded-2xl border border-emerald-100 space-y-3">
                        <div class="text-xs font-black text-emerald-700 uppercase flex items-center gap-1.5">
                            <i class="fa-solid fa-gift"></i> Hadiah Gratis (Gratis Y)
                        </div>
                        <div class="grid grid-cols-3 gap-3">
                            <div class="col-span-2">
                                <label class="block text-[10px] font-bold text-slate-500 mb-1 uppercase">Pilih Produk Gratis</label>
                                <select x-model="form.get_product_id" class="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 outline-none focus:border-primary font-bold text-xs cursor-pointer">
                            ...
                            <div>
                                <label class="block text-[10px] font-bold text-slate-500 mb-1 uppercase">Jumlah (Qty)</label>
                                <input type="number" min="1" x-model="form.get_qty" class="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 outline-none focus:border-primary font-bold text-xs text-center">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <label for="isActiveCheck" class="text-sm font-bold text-slate-700 cursor-pointer">Promo ini Aktif dan dapat digunakan di Kasir</label>
                    </div>
                </div>
